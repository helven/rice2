<x-filament::page>
    <div class="space-y-6">
        ...
        <x-filament::section>
            ...
            <div class="space-y-2 {{ $this->getDeliveryData()['is_today'] ? 'bg-yellow-100 dark:bg-yellow-900/30 p-4 rounded' : '' }}">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Delivery No:</span>
                        <span class="text-sm text-gray-900 dark:text-white ml-2">{{ $this->getDeliveryData()['delivery_no'] }}</span>
                    </div>
                    <div>
                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Delivery Date:</span>
                        <span class="text-sm text-gray-900 dark:text-white ml-2">{{ $this->getDeliveryData()['delivery_date'] }}</span>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Arrival Time:</span>
                        <span class="text-sm text-gray-900 dark:text-white ml-2">{{ $this->getDeliveryData()['arrival_time'] }}</span>
                    </div>
                    <div>
                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">DropOff Time:</span>
                        <span class="text-sm text-gray-900 dark:text-white ml-2">{{ $this->getDeliveryData()['dropoff_time'] }}</span>
                    </div>
                </div>
                <div class="grid grid-cols-1 gap-2">
                    <div>
                        ...
                        <span class="ml-2">
                            <div class="flex gap-1.5 flex-wrap">
                                <div class="flex w-max">
                                    @php
                                        $color = $this->getDeliveryData()['status_id'] == 1 ? 'warning' : ($this->getDeliveryData()['status_id'] == 2 ? 'success' : 'gray');
                                    @endphp
                                    <span style="--c-50:var(--{{ $color }}-50);--c-400:var(--{{ $color }}-400);--c-600:var(--{{ $color }}-600);" class="fi-badge flex items-center justify-center gap-x-1 rounded-md text-xs font-medium ring-1 ring-inset px-2 min-w-[theme(spacing.6)] py-1 fi-color-custom bg-custom-50 text-custom-600 ring-custom-600/10 dark:bg-custom-400/10 dark:text-custom-400 dark:ring-custom-400/30 fi-color-{{ $color }}">
                                        <span class="grid"><span class="truncate">{{ $this->getDeliveryData()['status'] }}</span></span>
                                    </span>
                                </div>
                            </div>
                        </span>
