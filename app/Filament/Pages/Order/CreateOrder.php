...
<?php
                    ...
                    //Grid::make(2)
                    //    ->schema([
                    //        DatePicker::make('delivery_start_date')
                    ...
                    //                $endDate = $get('delivery_end_date');
                    //                // If end date is blank or earlier than the new start date, update end date
                    //                if (blank($endDate) || \Carbon\Carbon::parse($endDate)->lt($state)) {
                    //                    $set('delivery_end_date', $state);
                    //                }
                    //            }),
                    //        DatePicker::make('delivery_end_date')
                    //            ->label('Delivery End Date')
                    //            ->placeholder('End Date')
                    //            ->required()
                    //            ->minDate(fn (callable $get) => $get('delivery_start_date') ?? now())
                    //            ->timezone('Asia/Kuala_Lumpur')
                    //            ->displayFormat('Y/m/d')
                    //            ->reactive()
                    //            ->afterStateUpdated(function ($state, callable $set, callable $get) {
                    //                $startDate = $get('delivery_start_date');
                    //                // If start date is blank or later than the new end date, update start date
                    //                if (blank($startDate) || \Carbon\Carbon::parse($startDate)->gt($state)) {
                    //                    $set('delivery_start_date', $state);
                    //                }
                    //            })
                    //        ]),
