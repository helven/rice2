...
trait OrderFormTrait
{
    ...
    protected function getDateSection(string $mode = 'order'): Section
    {
    ...
    protected function getMealSection(string $mode = 'order'): Section
    {
        if ($mode === 'create_order') {
            return Repeater::make('meals_by_date')
                ->label('')
                ->reorderable(false)
                ->deletable(true)
                ->disableItemCreation()
